<div>
    ...
    <div class="mb-6">
        <div class="relative">
            <input 
                ...
                placeholder="Cari supplier berdasarkan nama, alamat, atau telepon..." 
    ...
    @if($search)
        <div class="mb-4 text-sm text-slate-600">
            Ditemukan <span class="font-semibold text-slate-900">{{ count($suppliers) }}</span> supplier
        </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 lg:px-6 py-3 text-xs font-medium tracking-wider text-left text-slate-500 uppercase">Alamat</th>
                            <th class="px-4 lg:px-6 py-3 text-xs font-medium tracking-wider text-left text-slate-500 uppercase">Telepon</th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @foreach($suppliers as $supplier)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-slate-900">{{ $supplier->alamat }}</div>
                                </td>
                                <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-slate-900">{{ $supplier->telepon }}</div>
                                </td>
                                <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-slate-900">{{ $supplier->created_at?->format('d M Y') ?? '-' }}</div>
                                </td>
        ...
        @elseif($search)
            <div class="p-8 text-center">
                ...
                <p class="text-slate-600">Tidak ada supplier yang cocok</p>
            </div>
        @else
            <div class="p-8 text-center">
                <i class="fas fa-store text-4xl mb-3 text-slate-300"></i>
                <p class="text-slate-600">Mulai cari supplier untuk menampilkan hasil</p>
            </div>
        @endif
    </div>
</div>
