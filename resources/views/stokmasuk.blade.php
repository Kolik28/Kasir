...
@section('content')
    <div id="stok-masuk" class="page px-4 py-8">
        <!-- Header -->
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">Manajemen Stok Masuk</h2>
                    <p class="text-sm text-slate-600 mt-1">Kelola data stok masuk</p>
                </div>
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white border border-slate-200 rounded-lg shadow-sm p-4 md:p-6">
                ...
                <!-- Modal body -->
                <form id="supplierForm" action="{{ route('stokmasuk.store') }}" method="POST" class="space-y-4 md:space-y-6 py-4 md:py-6">
                    @csrf
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-slate-900">Nama Produk</label>
                        <select name="produk_id" id="produk_id" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" required>
                    ...
                    <div>
                        <label for="alamat" class="block mb-2 text-sm font-medium text-slate-900">Supplier</label>
                        <select name="supplier_id" id="supplier_id" class="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" required>
                    ...
                    <div>
                        <label for="telepon" class="block mb-2 text-sm font-medium text-slate-900">Jumlah Stok</label>
